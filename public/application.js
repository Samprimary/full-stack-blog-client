...
var createPlaylist = function createPlaylist(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.createPlaylistSubmit(data).then(ui.createPlaylistSuccess)
  // .then(ui.getPlaylistSuccess)
  .catch(ui.createPlaylistError);
  $(event.target).trigger('reset');
};

var getPlaylist = function getPlaylist(event) {
  event.preventDefault();
  // const data = getFormFields(event.target)
  api.getPlaylistSubmit().then(ui.getPlaylistSuccess).catch(ui.getPlaylistError);
};

var findPlaylist = function findPlaylist(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.findPlaylistSubmit(data);
};

var showUpdatePlaylist = function showUpdatePlaylist(event) {
  $('.update-playlist').show();
};
var updatePlaylist = function updatePlaylist(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  var playlistId = $(event.target).closest('ul').attr('data-id');
  api.updatePlaylistSubmit(data, playlistId).then(ui.updatePlaylistSuccess).catch(ui.updatePlaylistError);
  $(event.target).trigger('reset');
};

var deletePlaylist = function deletePlaylist(event) {
  event.preventDefault();
  var playlistId = $(event.target).closest('ul').attr('data-id');
  api.deletePlaylistSubmit(playlistId).then(ui.deletePlaylistSuccess).catch(ui.deletePlaylistError);
};
...
var playlistEvents = __webpack_require__(4);
...
var createPlaylistSubmit = function createPlaylistSubmit(data) {
  return $.ajax({
    method: 'POST',
...
var getPlaylistSubmit = function getPlaylistSubmit() {
  return $.ajax({
    method: 'GET',
    url: 'https://thawing-hamlet-16855.herokuapp.com/playlists/',
    headers: {
...
var findPlaylistSubmit = function findPlaylistSubmit(data) {
  return $.ajax({
    method: 'GET',
...
var updatePlaylistSubmit = function updatePlaylistSubmit(data, playlistId) {
  return $.ajax({
    method: 'PATCH',
...
var deletePlaylistSubmit = function deletePlaylistSubmit(playlistId) {
  return $.ajax({
    url: 'https://thawing-hamlet-16855.herokuapp.com/playlists/' + playlistId,
    method: 'DELETE',
    headers: {
...
var createPlaylistSuccess = function createPlaylistSuccess(response) {
  $('#display').html('Playlist created successfully!');
};

var createPlaylistError = function createPlaylistError(error) {
  $('#content').html('Playlist creation unsuccessful', error);
};

// use handlebars for below
var getPlaylistSuccess = function getPlaylistSuccess(data) {
  // console.log('response is ', response)
  $('#display').html('Playlists retrieved successfully!');
  var showPlaylistsHtml = handlebars({ playlists: data.playlists });
  $('#content').html('');
  $('#content').append(showPlaylistsHtml);
};
// use handlebars for above

var getPlaylistError = function getPlaylistError(error) {
  $('#display').html('Playlist retrieval unsuccessful', error);
};

var deletePlaylistSuccess = function deletePlaylistSuccess(playlistId) {
  $('#display').html('Playlist deleted successfully!');
  $('#content').html('');
};
var deletePlaylistError = function deletePlaylistError(error) {
  $('#display').html('Cannot delete playlist', error);
};

var findPlaylistSuccess = function findPlaylistSuccess(response) {
  $('#display').html('Playlist found successfully!');
};
var findPlaylistError = function findPlaylistError(error) {
  $('#display').html('Playlist find unsuccessful', error);
};

var updatePlaylistSuccess = function updatePlaylistSuccess(playlistId) {
  $('#display').html('Playlist updated successfully!');
  $('#content').html('');
};
var updatePlaylistError = function updatePlaylistError(error) {
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "  "
    ...
    + "\n  <div class=\"col-sm-3 playlist-box\">\n    <h2 data-id=\""
    ...
    + "</h3></li>\n      <li>\n        <form class=\"update-playlist\">\n          <input name=\"playlist[title]\" type=\"text\" placeholder=\"title\">\n          <br>\n          <input name=\"playlist[description]\" type=\"text\" placeholder=\"description\">\n          <br><br>\n          <button type=\"submit\" class=\"btn btn-primary\">Update Playlist!</button>\n        </form>\n      </li>\n      <li><button  class=\"delete-playlist btn btn-danger\">Delete Playlist</button></li>\n    </ul>\n</div>\n\n";
  ...
  return ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.playlists : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "");
